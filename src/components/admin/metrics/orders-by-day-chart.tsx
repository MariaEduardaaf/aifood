...
export function OrdersByDayChart({ periodo = 'semana' }: OrdersByDayChartProps) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setLoading(true)
        const response = await fetch(`/api/admin/metricas/pedidos/dia-semana?periodo=${periodo}`)
          ...
          <BarChart data={translatedData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
            ...
            <Tooltip
              ...
              formatter={(value, name) => {
                if (name === 'receita') return [formatCurrency(value as number), t('revenue')]
                return [value as number, t('orders')]
              }}
            />
            <Legend
              formatter={(value) => value === 'quantidade' ? t('orders') : t('revenue')}
            />
            <Bar
              yAxisId="left"
              dataKey="quantidade"
