...
export function TopProductsChart({ periodo = 'hoje', limit = 10 }: TopProductsChartProps) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        setLoading(true)
        const response = await fetch(`/api/admin/metricas/produtos/top?periodo=${periodo}&limit=${limit}`)
        ...
        {data.slice(0, 5).map((product, index) => (
          <div key={product.id} className="flex items-center justify-between py-2 border-b border-gray-100 dark:border-gray-700 last:border-0">
            <div className="flex items-center gap-3">
              ...
              />
              <div>
                <p className="text-sm font-medium text-gray-900 dark:text-white">{product.nome}</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">{product.categoria}</p>
              </div>
            </div>
            <div className="text-right">
              <p className="text-sm font-medium text-gray-900 dark:text-white">{product.quantidade} {t('units')}</p>
              <p className="text-xs text-gray-500 dark:text-gray-400">{formatCurrency(product.receita)}</p>
            </div>
          </div>
        ))}
      </div>
