  ...
  try {
    ...
    if (resolvedCalls.length > 0) {
      const totalTime = resolvedCalls.reduce((sum, call) => {
        const responseTime = (new Date(call.resolved_at!).getTime() - new Date(call.created_at).getTime()) / 1000
    ...
    const slaPercentage = resolvedCalls.length > 0
    ...
    // Get waiter names
    const waiterIds = callsByWaiter.map(c => c.resolved_by).filter(Boolean) as string[]
